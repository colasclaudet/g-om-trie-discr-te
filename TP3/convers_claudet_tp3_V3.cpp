...
std::vector<ContourPol> * homcon(std::vector<ContourPol> * vect_cp, ContourPol cp1, int idx_cp1,
 ContourPol cp2, int idx_cp2, double distance, double seuil)
{
	...
	for(int i = idx_cp1; i < idx_cp2; i++)
	{
		double d = PerpendicularDistance(vect_cp->at(i).p, vect_cp->at(idx_cp1).p, vect_cp->at(idx_cp2).p);
		if (d > dmax)
		{
			index = i;
			dmax = d;
...
void colorier_morceaux(ContourF8 cfc,std::vector<ContourPol> v,cv::Mat img)
{
  int cpt = 1;
...
std::vector<ContourPol> approximer_contour_c8(ContourF8 cfc, cv::Mat img)
{
	...
	std::vector<ContourPol> vect_tmp;
	std::cout<<"PATATE NATURE"<<std::endl;
	std::cout<<vect_contour_pol.size()<<std::endl;
	if(vect_contour_pol.size()<3 || vect_contour_pol.size()>300){return vect_contour_pol;}
  colorier_morceaux(cfc,vect_contour_pol,img);
	for(unsigned int i = 0 ;i < vect_contour_pol.size(); i++)
	{

		std::cout<<"RACLETTE"<<i<<std::endl;
		std::cout<< " x : " <<vect_contour_pol.at(i).p.x << " y : " <<vect_contour_pol.at(i).p.y << " value : " <<vect_contour_pol.at(i).estSommetApproxPoly;

		if(vect_contour_pol.at(i).estSommetApproxPoly)
		{
			vect_tmp.push_back(vect_contour_pol.at(i));
		}
	}
	std::cout<<"PATATE AU MIEL"<<std::endl;
	for(unsigned int i = 0 ; i<vect_tmp.size()-2; i++)
	{
		std::cout<<"vector size -2 : "<<vect_tmp.size()-2<<std::endl;
		std::cout<<"vector x :  "<<vect_tmp.at(i).p.x<<" vector y :  "<<vect_tmp.at(i).p.y<<std::endl;
		if(distance_max_poly(vect_tmp,vect_tmp.at(i),i,vect_tmp.at(i+2),i+2)>seuil)
		{
			std::cout<<"dmax  "<<distance_max_poly(vect_tmp,vect_tmp.at(i),i,vect_tmp.at(i+2),i+2)<<std::endl;
			vect_tmp.at(i+1).estSommetApproxPoly = false;
		}
	}
	std::cout<<"PATATE AU BEURRE"<<std::endl;
	std::vector<point_img> vt_pt;
	for(unsigned int i = 0 ; i<vect_tmp.size(); i++)
	{
		if(vect_tmp.at(i).estSommetApproxPoly)
		{
			std::cout<<" est somme approx poly "<<std::endl;
			vt_pt.push_back(vect_tmp.at(i).p);
		}
	}
	for(unsigned int i = 0 ; i<vt_pt.size()-1; i++)
	{
		std::cout<<"_______________modif img : ________________"<<" X1 "<<
		vt_pt.at(i).x<<" Y1 "<<vt_pt.at(i).y<<" X2 "<<vt_pt.at(i+1).x<<" Y2 "<<vt_pt.at(i+1).y<<std::endl;
		//bresmhan(img, vt_pt.at(i).x, vt_pt.at(i).y, vt_pt.at(i+1).x, vt_pt.at(i+1).y);
	}
	max_iter++;
	return vect_contour_pol;
}
...
void dessiner_approx_poly(cv::Mat img)
{
	...
	//for(int i = 0;i<2;i++)
	{
		...
		approximer_contour_c8(contours.at(i), img);
...
int main (int argc, char**argv)
{
    ...
    cv::createTrackbar ("Seuil", "ImageSrc", &my.seuil, 255,
        onSeuilSlide, &my);
    cv::createTrackbar ("Seuil2", "ImageSrc", &my.seuil, 1000,
            onSeuilSlide2, &my);
