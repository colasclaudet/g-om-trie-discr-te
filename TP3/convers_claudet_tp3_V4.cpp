...
void homcon2(std::vector<ContourPol> * vect_cp, int idx_cp1, int idx_cp2, double seuil)
 {
   ...
   /*---------------------------------------------------------
   int x1 = vect_cp->at(idx_cp1).p.x;
   int y1 = vect_cp->at(idx_cp1).p.y;

   int x2 =  vect_cp->at(idx_cp2).p.x;
   int y2 =  vect_cp->at(idx_cp2).p.y;
   dis.distance = 0;
   int r;
   int xc;
   int yc;
   for(unsigned int i=idx_cp1 ; i<=idx_cp2 ; i++)
   {
     xc = vect_cp->at(i).p.x;
     yc = vect_cp->at(i).p.y;
     float dist = abs(((x2-x1)*(yc-y1) - (y2-y1)*(xc-x1)))
   ...
   *///-------------------------------------------------------------
   ...
   {
     for(unsigned int i = idx_cp1+1; i<=idx_cp2;i++)
     {
       vect_cp->at(i).estSommetApproxPoly = false;
     }
     return;
