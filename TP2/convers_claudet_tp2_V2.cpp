...
cv::Mat marquer_un_contour_c8(cv::Mat img,int xa,int ya,int dira,int num_contour)
{
	...
	do
	{
		...
		for(;i<8;i++)
		{
			...
			int q = next(img,y,x,d);
			int xbis = x + dir_x[d];
			int ybis = y + dir_y[d];
			std::cout << "    for"<<std::endl;
			//if(est_dans_image(img,q) && q>0)
			if(q>0 && (dir_x[dir]+x)>=0 && (dir_y[dir]+y)>=0 
			&& (dir_x[dir]+x)<=img.rows-1 && (dir_y[dir]+y)<=img.cols-1)
			{
				x = dir_x[dir]+x;
				y = dir_y[dir]+y;
				//x=xbis;
...
cv::Mat suivre_un_contour_c8(cv::Mat img,int xa,int ya,int dira,int num_contour)
{
	...
	for(int i = 0; i<8; i++)
	{
		...
		if(next(img,ya,xa,d) >= 0 && (dir_y[d]+ya)>=0 && (dir_x[d]+xa)>=0 
			&& (dir_x[d]+xa)<=img.rows-1 && (dir_y[d]+ya)<=img.cols-1)
		{
			dir_finale = (d+4)%8;
			break;
		}
	}
	return marquer_un_contour_c8(img, xa,ya,dir_finale,num_contour);
	//return img;
}
void effectuer_suivi_contours_c8(cv::Mat img_niv)
{
	int new_contour = 1;
	for(int x = 0; x< img_niv.rows-1; x++)
	{
		for (int y = 0; y < img_niv.cols-1; y++)
		{
			std::cout<< "pixel : " << x << ", "<< y << std::endl;
			if(img_niv.at<int>(x,y) == 255)
			{
				int dir = -1;
				if(y==img_niv.cols-1 || img_niv.at<int>(x,y+1) == 0)
				{
					dir = 0;
				}
				else if(x==img_niv.rows-1 || img_niv.at<int>(x+1,y) == 0)
				{
					dir = 2;
				}
				...
				if(dir>=0)
				{
					suivre_un_contour_c8(img_niv,x,y,dir,new_contour);
					new_contour++;
					if(new_contour == 255)
					{
						new_contour++;
